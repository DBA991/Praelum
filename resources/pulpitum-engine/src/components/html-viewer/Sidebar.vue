...
const props = defineProps({
  ...
  noGroupSelectedMessage: {
    type: String,
    default: 'Seleziona un gruppo per visualizzarne il contenuto.'
  },

  navigationLabels: {
    type: Object,
    default: () => ({
      previous: 'Gruppo precedente',
      next: 'Gruppo successivo'
    })
  },
...
const getEmptyGroupMessage = () => {
  if (!hasGroups.value || activeGroupId.value === null) return props.emptyListMessage

  const group = props.groups.find(
    (g) => (g.slug || props.groups.indexOf(g)) === activeGroupId.value
  )
  return group ? `Nessun elemento in ${group.name.toLowerCase()}.` : props.emptyListMessage
}
...
<template>
  <aside class="sidebar" :class="[sidebarClass, { 'is-visible': isVisible }]">
    <div class="sidebar-header">
      ...
      <!-- Slot per controlli personalizzati (es. navigation buttons) -->
      <slot name="controls"></slot>

      <!-- Navigation buttons integrati (se più di un elemento in groups) -->
    ...
    <div class="sidebar-content">
      <!-- Lista singola (modalità semplice) -->
      ...
      <!-- Liste multiple (modalità gruppi) -->
      <template v-else>
