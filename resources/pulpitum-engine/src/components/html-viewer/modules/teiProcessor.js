...
function resolveReference(targetValue, contentArea) {
  ...
  if (targetValue.startsWith('http://') || targetValue.startsWith('https://') || targetValue.startsWith('www.')) {
...
function preserveElementStructure(element) {
  ...
  for (const attr in element.dataset) {
    if (attr !== 'tag' && attr !== 'annotated' && Object.prototype.hasOwnProperty.call(element.dataset, attr)) {
...
export function extractAllLists(htmlContent) {
  ...
  if (teiHeaderElement) {
    const specificListElements = teiHeaderElement.querySelectorAll('[data-tag^="list"]:not([data-tag="list"])');
    ...
    genericListElements.forEach(listEl => {
      ...
      listEl.querySelectorAll('[data-tag="item"]').forEach((itemEl, index) => {
        ...
        const id = itemEl.dataset.id || itemEl.id || `_auto_generic_${listType}_${Date.now()}_${index}`;
        const structure = preserveElementStructure(itemEl);
        const display = createGenericDisplay(structure);
        items.push({
          id,
...
function formatListName(listTag) {
  ...
  return nameMap[listTag] || listTag.replace('list', '').charAt(0).toUpperCase() + listTag.replace('list', '').slice(1);
}