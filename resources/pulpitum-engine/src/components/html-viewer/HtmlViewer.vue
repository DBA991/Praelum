...
const NARROW_PANEL_THRESHOLD = 992
...
const hasAnyList = computed(() => allTeiList.value.length > 0)


const checkScreenSize = (width) => {
  isSmallScreen.value = width < NARROW_PANEL_THRESHOLD
...
onMounted(async () => {
  ...
  updateContent(props.htmlContent)

  
  watch(
    () => props.htmlContent,
    (newHtml) => updateContent(newHtml)
  )
  ...
  if (rootEl.value) {
    resizeObserver = new ResizeObserver((entries) => {
      ...
      if (entry) checkScreenSize(entry.contentRect.width)
    })
    resizeObserver.observe(rootEl.value)
    checkScreenSize(rootEl.value.clientWidth)
  }
...
const updateContent = (html) => {
  ...
  nextTick(() => {
    ...
    const { lists, pages } = teiProcessor.extractAllLists(html)
    allTeiList.value = lists
...
const updateActivePage = () => {
  ...
  if (activePageIndex.value !== newActivePageIndex) {
    ...
    const pageN = pageList.value[newActivePageIndex]?.n
    if (pageN !== undefined) {
      emit('page-change', String(pageN))
    }
  }
}
...
const handleDisplayListItem = ({ item, group }) => {
...
const goToPage = (pageN) => {
  if (!contentArea.value) return
  const n = String(pageN)
  const targetElement = contentArea.value.querySelector(`[data-tag="pb"][data-n="${n}"]`)
  if (targetElement) {
    targetElement.scrollIntoView({ behavior: 'smooth', block: 'start' })
    nextTick(updateActivePage)
  }
}
...
<template>
  <div class="html-viewer-container" ref="rootEl" :class="{ 'narrow-panel': isSmallScreen }">
    <SearchSection
    ...
    <!-- Pulsante toggle Indice delle Pagine (mobile) -->
    <button
    ...
    <!-- Pulsante toggle Liste TEI (mobile) -->
    <button
    ...
    <div v-if="hasHeaderAvailable" class="header-section">
      <button class="header-btn" @click="showTeiHeader" aria-label="Mostra informazioni documento">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="18"
          height="18"
...
.narrow-panel .main-content-wrapper {
  overflow-x: hidden;
}

.narrow-panel :deep(.page-sidebar),
.narrow-panel :deep(.metadata-sidebar) {
  position: absolute;
  top: 0;
  height: 100%;
  width: 280px;
...
.narrow-panel :deep(.page-sidebar.sidebar-visible),
.narrow-panel :deep(.metadata-sidebar.sidebar-visible) {
  transform: translateX(0);
}

.narrow-panel .viewer-content {
  order: 0;
}
</style>
